...
async function main() {
  try {
    ...
    // Seed demo repos in background (non-blocking)
    import('./services/seed.js').then(({ run }) => {
      run().catch((err) => logger.warn({ err }, 'SeedService failed (non-fatal)'));
    }).catch((err) => logger.warn({ err }, 'Failed to import SeedService'));
