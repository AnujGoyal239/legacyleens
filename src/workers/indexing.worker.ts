...
import { computeHealthScore } from '../server/services/healthScoreCompute.js';
    ...
    try {
      ...
      try {
        ...
        const healthResult = computeHealthScore(
          projectFiles as Parameters<typeof computeHealthScore>[0],
          entryPoints
        );

        await (prisma as any).codebaseHealthScore.upsert({
          where: { projectId },
          create: {
            projectId,
            overallScore: healthResult.overallScore,
            docCoverageScore: healthResult.docCoverageScore,
            criticalFileRiskScore: healthResult.criticalFileRiskScore,
            complexityScore: healthResult.complexityScore,
            onboardingReadiness: healthResult.onboardingReadiness,
          },
          update: {
            overallScore: healthResult.overallScore,
            docCoverageScore: healthResult.docCoverageScore,
        ...
        logger.info(
          { projectId, overallScore: healthResult.overallScore },
          '[Indexing] Health score computed and stored'
        );
      } catch (healthError) {
        logger.warn(
          { projectId, error: healthError instanceof Error ? healthError.message : String(healthError) },
          '[Indexing] Health score computation failed (non-fatal)'
        );
      }
